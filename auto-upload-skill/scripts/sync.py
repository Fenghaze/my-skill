...
def generate_readme(skills):
    ...
    table_lines = []
    if skills:
        table_lines.append("| Name | Description | Version | Author |")
        table_lines.append("|------|-------------|---------|--------|")
        for skill in skills:
            name = skill.get('name', '')
            ...
            table_lines.append(f"| {name} | {desc} | {version} | {author} |")

    skills_table = '\n'.join(table_lines)

    # 构建Individual Skill Docs
    docs_lines = []
    for skill in skills:
        name = skill.get('name', '')
        desc = skill.get('description', '')
        docs_lines.append(f"### {name}")
        docs_lines.append(f"_{desc}_")
        docs_lines.append("")

        # 读取skill的SKILL.md前30行作为预览
        skill_md_path = Path(skill.get('path', '')) / "SKILL.md"
        if skill_md_path.exists():
            lines = skill_md_path.read_text(encoding='utf-8').split('\n')[:30]
            docs_lines.append("```markdown")
...
```bash
git clone https://github.com/{github_repo}.git ~/.claude/skills-temp
