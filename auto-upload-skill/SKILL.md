...
自动化skill和command同步工具 - 当您在 `~/.claude/skills/` 或 `~/.claude/commands/` 目录创建或修改内容时，自动将其上传到 GitHub 仓库。

## 功能

- **定时自动同步**：支持24小时（可配置）自动上传所有skills和commands到GitHub
- 文件监视：持续监控 `~/.claude/skills/` 和 `~/.claude/commands/` 目录变化
- **交互确认**：检测到skill或command变化时，先询问是否上传，确认后执行Git add/commit/push
- **自动生成README**：每次同步后自动更新仓库README，包含所有skills的描述和使用方法，以及commands列表
...
| commandsDir | 监视的commands目录 | ~/.claude/commands |
...
3. 将所有skills复制到本地git仓库的 `skills/` 目录
4. 将所有commands复制到本地git仓库的 `commands/` 目录
5. 解析所有skill的SKILL.md，生成/更新README.md
6. 执行 git add → commit → push
7. 记录下次同步时间，可通过 Ctrl+C 停止

### 交互式监视模式 (--daemon)
1. daemon使用 watchdog 监视 `~/.claude/skills/*/SKILL.md` 和 `~/.claude/commands/*` 文件
2. 检测到变化后，等待5秒防抖
3. **询问用户是否确认上传**（跳过自动上传）
4. 用户确认后，将变化的skill或command复制到本地git仓库对应目录
